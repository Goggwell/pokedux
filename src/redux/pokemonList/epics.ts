...
const fetchPageEpic: TypedEpic = (action$, state$, { observableFetch }) => {
    return action$.pipe(
        ...
        switchMap(action => {
            const urlData = "url" in action.payload ? getUrlDataByAddress(action.payload.url) : getDefaultUrlData(action.payload.offset, action.payload.size);

            return observableFetch<INamedApiResourceList<IPokemon>>(urlData.url)
                    .pipe(mergeMap(json => new Observable<AnyAction>(subscriber => { 
                        subscriber.next(actions.pageFetched({list: json, size: urlData.size, offset: urlData.offset}));
                        for (const result of json.results) {
                            subscriber.next(actions.fetchDetails(result.name));
                        }
                    })
                ),
                catchError(error => of(actions.setError(error instanceof ApiError && error.message === "404" ? "Nothing found" : error.message || error)))
            );
        })
    );
};
...
const fetchPokemonDetailsEpic: TypedEpic = (action$, state$, { observableFetch }) => {
    return action$.pipe(
        ...
        mergeMap(action => {
            ...
            return observableFetch<IPokemon>(url).pipe(map(jsonResult => actions.detailsFetched({pokemonName: action.payload, data: jsonResult})),
                catchError(error => of(actions.setDetailsError({pokemonName: action.payload, error: error instanceof ApiError && error.message === "404" ? "Nothing found" : error.message || error}))),
