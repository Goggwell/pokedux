    ...
    reducers: {
        ...
        pageFetched: (
            ...
            action: PayloadAction<{
                list: INamedApiResourceList<IPokemon>;
                size: number;
                offset: number;
            }>
        ) => {
            ...
            state.pageCount = Math.ceil(action.payload.list.count / action.payload.size);
            state.currentPage = state.pageCount - Math.ceil((action.payload.list.count - action.payload.offset) / action.payload.size) + 1;
        },
